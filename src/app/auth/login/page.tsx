  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-950 via-green-900 to-emerald-800 flex items-center justify-center px-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden">
        <div className="bg-gradient-to-r from-green-700 to-emerald-600 px-8 py-5 text-center">
          <div className="flex items-center justify-center gap-2 mb-0.5">
            <TreePine className="text-white" size={22} />
            <span className="font-bold text-white text-xl">CamperWatch</span>
          </div>
          <p className="text-green-100 text-xs">The community for real campers</p>
        </div>
        <div className="p-6">
          {urlError && (
          ...
          {/* ── MOBILE: magic link ONLY — fastest path, no decisions ── */}
          <div className="sm:hidden">
            <h1 className="text-lg font-bold text-gray-900 text-center mb-1">Sign in</h1>
            <p className="text-gray-400 text-xs text-center mb-5">We'll email you a one-tap link — no password needed</p>
            <form onSubmit={handleMagicLink} className="space-y-3">
              <div className="relative">
                <Mail size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="your@email.com" required
                  className="w-full pl-10 pr-4 py-3.5 border border-gray-200 rounded-2xl text-base focus:outline-none focus:ring-2 focus:ring-green-500" />
              </div>
              {error && <p className="text-red-500 text-xs">{error}</p>}
              <button type="submit" disabled={loading || !email}
                className="w-full bg-green-600 text-white py-3.5 rounded-2xl font-bold text-base transition-colors disabled:opacity-40 flex items-center justify-center gap-2">
                {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <><Sparkles size={16}/> Send magic link</>}
              </button>
            </form>
            <div className="mt-4 text-center">
              <button onClick={handleGoogle} className="text-xs text-gray-400 underline underline-offset-2">Use Google instead</button>
            </div>
            <p className="text-center text-sm text-gray-400 mt-4">
              New here?{' '}
              <Link href={`/auth/signup?redirect=${encodeURIComponent(redirect)}`} className="text-green-700 font-semibold">Join free</Link>
            </p>
          </div>

          {/* ── DESKTOP: full options — Google + magic/password toggle ── */}
